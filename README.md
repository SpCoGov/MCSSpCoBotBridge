MSSBB(**M**inecraft**S**erver-**S**pCo**B**ot**B**ridge Minecraft服务器-SpCoBot桥接器)
是一个仅服务端需安装的mod，用于让SpCoBot与Minecraft服务器进行通信，如向服务器发送命令并获取返回值、订阅事件等。
...
| 6 | Heartbeat ACK | Receive | 当MSBBS收到心跳包时，发送此消息 |
...
| 字段  | 描述                                                                     |
|-----|------------------------------------------------------------------------|
| syn | 同步码，必须为整数，可以省略。如果填写了该字段，MSSBB在返回Heartbeat ACK消息时，会在数据中附上`ack`字段且值等于同步码 |

MSSBB收到心跳后会返回Heartbeat ACK消息，如下：
...
| 字段  | 描述                                   |
|-----|--------------------------------------|
| ack | 确认码，值等于心跳的同步码，如果心跳的同步码省略，则该消息也不包含确认码 |
